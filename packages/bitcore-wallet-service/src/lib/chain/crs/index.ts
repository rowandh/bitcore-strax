...
export class CrsChain implements IChain {
  protected sizeEstimationMargin: number;
  protected inputSizeEstimationMargin: number;

  constructor(private bitcoreLib = BitcoreLib) {
    this.sizeEstimationMargin = config.strax?.sizeEstimationMargin ?? 0.01;
    this.inputSizeEstimationMargin = config.strax?.inputSizeEstimationMargin ?? 2;
  }
